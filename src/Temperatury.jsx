...
import GotoweTemperatury from './GotoweTemperatury'; // Zaimportuj komponent Gotowce
  ...
  const obliczObjetosc2 = () => {
    ...
    if (!isNaN(V1) && !isNaN(T1) && !isNaN(T2) && !isNaN(T_mieszaniny)) {
      ...
      if (T2 - T_mieszaniny !== 0) {
        ...
        alert('Nie można obliczyć objętości drugiej cieczy. Upewnij się, że temperatura drugiej cieczy jest różna od żądanej temperatury mieszaniny.');
      }
    } else {
  ...
  return (
    ...
    <Container maxWidth="md">
      <Box sx={{ my: 4 }}>
        ...
        <Button variant="outlined" onClick={() => navigate('/GotoweTemperatury')}> {/* Dodaj przycisk Gotowce */}
        GotoweTemperatury
          </Button>
        {objetosc2 !== null && (
          ...
          <Typography variant="body1" mt={1}>
              Łączna objętość mieszaniny: {(parseFloat(objetosc1) + objetosc2).toFixed(2)} l
            </Typography>
          </Box>
        </Container>
